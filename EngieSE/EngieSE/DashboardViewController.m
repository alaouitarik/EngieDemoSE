...
- (void)viewDidLoad {
    ...
    [logoImage setFrame:CGRectMake(0, 0, 82, 30)];
...
-(void) initContentRequest{
    ...
    NSString *resultat = [[NSUserDefaults standardUserDefaults] stringForKey:@"resultSubscription"];
...
-(void) openViewController:(NSString *) VCName{
    UIViewController *destinationVC = [self.storyboard instantiateViewControllerWithIdentifier:VCName];
    UIViewController *topVC = self.navigationController.topViewController;


    if (topVC.restorationIdentifier == destinationVC.restorationIdentifier){
        NSLog(@"Same VC");
    } else {
